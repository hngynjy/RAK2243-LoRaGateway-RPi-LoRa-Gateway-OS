...
do_configure_append() {
    cp ${WORKDIR}/lora_pkt_fwd.c ${S}/lora_pkt_fwd/src/lora_pkt_fwd.c
    cp ${WORKDIR}/update_gwid.sh ${S}/lora_pkt_fwd/update_gwid.sh
}

do_compile() {
    oe_runmake
    ${CC} ${CFLAGS} ${LDFLAGS} ${WORKDIR}/gps.c  -o gpsx 
}
...
do_install() {
    ...
    install -m 0755 ${WORKDIR}/start.sh ${D}${LORA_DIR}/
    install -m 0755 ${S}/gpsx ${D}${LORA_DIR}/
    ...
    install -m 0644 ${WORKDIR}/rc.local ${D}${sysconfdir}/rc.local
