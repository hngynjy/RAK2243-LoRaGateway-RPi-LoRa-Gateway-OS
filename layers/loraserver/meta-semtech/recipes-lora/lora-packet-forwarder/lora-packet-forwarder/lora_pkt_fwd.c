...
//jim        i = lgw_gps_enable(gps_tty_path, "ubx7", 0, &gps_tty_fd); /* HAL only supports u-blox 7 for now */
...
uint8_t ubx_cmd_timegps[UBX_MSG_NAVTIMEGPS_LEN] = {
  ...
  gps_tty_fd = open(filename, O_RDWR);
  if (file < 0) {
    exit(1);
  }

  int addr = 0x42; /* I2C slave address */
  if (ioctl(gps_tty_fd, I2C_SLAVE, addr) < 0) {
 
    exit(1);
  }

    num_written = write (gps_tty_fd, ubx_cmd_timegps, UBX_MSG_NAVTIMEGPS_LEN);
